...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  html, body { background: #030610; color: ${G.textPrimary}; font-family: 'Poppins', sans-serif; -webkit-font-smoothing: antialiased; }
...
const AdminDashboard: React.FC = () => {
  ...
  const analytics = useMemo(() => {
    ...
    // ── FIXED: chart data uses real completionRate, shows 0 when no tasks ──
...
// ── Sync task updates to backend ──────────────────────────────────────────
  ...
  const handleApprove = (): void => {
   ...
   const handleRework = (): void => {
    ...
    const h: HistoryEntry = { id: `hist_${Date.now()}`, timestamp: new Date().toISOString(), action: "rejected", by: user?.email ?? "", notes: reviewComments };
    const updatedTask = { ...selectedTask, history: [...(selectedTask.history ?? []), h] };
    updateTask(selectedTask.id, updatedTask as never);
    syncTaskToBackend(updatedTask as Task);
  ...
  return (
    <>
      ...
      {useImageBackground && backgroundImage ? (
        ...
        <video style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", objectFit: "cover", zIndex: -1, opacity: 0.40 }}
          autoPlay muted loop playsInline preload="auto">
          <source src="/videos/GettyImages-1370505974.mp4" type="video/mp4" />
        </video>
      )}

      <div style={{ position: "fixed", inset: 0, zIndex: -1, background: "linear-gradient(160deg, rgba(2,4,16,0.55) 0%, rgba(4,8,24,0.45) 50%, rgba(2,4,16,0.60) 100%)", pointerEvents: "none" }} />
      ...
      <div style={{ minHeight: "100vh", position: "relative" }}>
        <div style={{ position: "relative", zIndex: 1, maxWidth: 1280, margin: "0 auto", padding: "0 28px" }}>
          ...
          {/* ── HEADER ── */}
          <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 0", position: "sticky", top: 0, zIndex: 50, background: "linear-gradient(90deg, rgba(2,4,16,0.75) 0%, rgba(6,12,32,0.72) 50%, rgba(2,4,16,0.75) 100%)", backdropFilter: "blur(28px)", borderBottom: "1px solid rgba(255,255,255,0.08)", boxShadow: `0 8px 32px rgba(0,0,0,0.4), inset 0 -1px 0 ${G.cyan}22` }}>
          ...
          {activeTab === "analytics" && (
            <section style={{ marginTop: 32, paddingBottom: 60 }}>
              ...
              {/* AREA CHART — now uses analytics.chartData (real data, 0 when no tasks) */}
