...
Current status: ${activeTasks.length} tasks | ${activeTasks.filter(t=>t.tatBreached).length} TAT breaches | ${activeTasks.filter(t=>t.status==="in_progress").length} in progress | Team: ${MOCK_USERS.length}
                ...
                <div className="card">
                  ...
                  <div className="card-body">
                    <ResponsiveContainer width="100%" height={210}>
                      <BarChart data={(() => {
                        const doers = MOCK_USERS.filter(u => u.isDoer);
                        return doers.map(u => {
              ...
              <div className="card">
                ...
                <div className="card-body">
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:12 }}>
                    {MOCK_USERS.filter(u => u.isDoer).map(u => {
                      const ut = activeTasks.filter(t => t.assignedTo === u.email);
                      ...
                      activeTasks.map((task, i) => (
                        <div key={i} style={{ display:"flex", gap:12, padding:"10px 12px", background:"var(--srf2)", borderRadius:8, borderLeft:`3px solid ${STATUS_COLOR[task.status]?.includes("green")?"var(--grn)":STATUS_COLOR[task.status]?.includes("sky")?"var(--sky)":STATUS_COLOR[task.status]?.includes("amber")?"var(--amber)":"var(--acc)"}` }}>
                          <div className={`avatar av-sm ${AV_COLORS[i % AV_COLORS.length]}`} style={{ flexShrink:0 }}>
                            {MOCK_USERS.find(u => u.email===task.assignedTo) ? initials(MOCK_USERS.find(u => u.email===task.assignedTo)!.name) : "?"}
                          </div>
                          <div style={{ flex:1 }}>
                            <div style={{ fontSize:13, color:"var(--t1)", fontWeight:500 }}>{task.title}</div>
                            <div style={{ fontSize:11, color:"var(--t2)", marginTop:2 }}>
                              {MOCK_USERS.find(u => u.email===task.assignedTo)?.name || task.assignedTo}
                      ...
                      {activeTasks.filter(t => t.tatBreached).length === 0 ? (
                        ...
                        <div key={i} style={{ padding:"8px 10px", background:"rgba(244,63,94,.06)", border:"1px solid rgba(244,63,94,.15)", borderRadius:6 }}>
                          ...
                          <div style={{ fontSize:10, color:"var(--t3)", marginTop:2 }}>
                            {MOCK_USERS.find(u=>u.email===t.assignedTo)?.name || t.assignedTo} · Due {t.dueDate}
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="card">
                  ...
                  <div className="card-body">
                    <ResponsiveContainer width="100%" height={220}>
                      <BarChart data={(() => {
                        return MOCK_USERS.filter(u => u.isDoer).map(u => {
                          const ut = activeTasks.filter(t => t.assignedTo === u.email);
                          return {
                            name: u.name.split(" ")[0],
                            completed: ut.filter(t => t.status==="completed"||t.status==="approved").length,
