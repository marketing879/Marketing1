...
const AV = [
  { bg: 'rgba(99,102,241,0.3)', c: '#a5b4fc' },
  { bg: 'rgba(34,197,94,0.25)', c: '#4ade80' },
  { bg: 'rgba(245,158,11,0.25)', c: '#fbbf24' },
  { bg: 'rgba(244,63,94,0.25)', c: '#fb7185' },
  { bg: 'rgba(14,165,233,0.25)', c: '#38bdf8' },
  { bg: 'rgba(168,85,247,0.25)', c: '#d8b4fe' },
...
function statusColor(s: string) {
  switch (s) {
    case 'approved': return { sc: '#4ade80', sb: 'rgba(34,197,94,0.12)' };
    case 'completed': return { sc: '#4ade80', sb: 'rgba(34,197,94,0.12)' };
...
.cc-root{display:flex;flex-direction:column;height:100vh;background:linear-gradient(135deg,#02040a 0%,#050b18 50%,#02040a 100%);color:#eef2ff;font-family:'DM Sans',sans-serif;overflow:hidden;}

.cc-topbar{display:flex;align-items:center;justify-content:space-between;padding:0 20px;height:54px;background:rgba(5,10,25,0.95);border-bottom:1px solid rgba(212,168,71,0.15);flex-shrink:0;position:relative;z-index:10;backdrop-filter:blur(20px);}
.cc-topbar::after{content:'';position:absolute;bottom:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(212,168,71,0.6),transparent);}

.cc-main{display:flex;flex:1;overflow:hidden;}
.cc-left{width:38%;display:flex;flex-direction:column;border-right:1px solid rgba(212,168,71,0.12);padding:12px;gap:9px;background:rgba(2,4,12,0.4);}
.cc-right{width:62%;display:flex;flex-direction:column;padding:12px;gap:9px;overflow-y:auto;background:rgba(4,8,22,0.3);}

.cc-video-main{flex:1;border-radius:14px;overflow:hidden;border:1px solid rgba(99,102,241,0.2);background:linear-gradient(135deg,#0a0f1e,#0d1117);position:relative;display:flex;align-items:center;justify-content:center;min-height:0;box-shadow:inset 0 0 40px rgba(99,102,241,0.05);}
.cc-thumbs{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;}
.cc-thumb{aspect-ratio:16/9;border-radius:8px;background:linear-gradient(135deg,rgba(16,22,36,0.9),rgba(8,12,24,0.9));border:1px solid rgba(255,255,255,0.06);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;transition:.2s;}
.cc-thumb:hover{border-color:rgba(99,102,241,0.3);}

.cc-toolbar{background:rgba(10,15,30,0.9);border-radius:10px;padding:7px 12px;display:flex;align-items:center;justify-content:space-between;border:1px solid rgba(255,255,255,0.07);flex-shrink:0;}
...
.cc-filter-btn{background:rgba(10,15,30,0.8);border:1px solid rgba(255,255,255,0.08);border-radius:8px;padding:5px 11px;font-size:11px;color:#8b9ab8;display:flex;align-items:center;gap:5px;cursor:pointer;transition:.2s;font-family:'DM Sans',sans-serif;position:relative;}
.cc-filter-btn:hover{border-color:rgba(212,168,71,0.3);color:#f0c060;}
.cc-filter-btn.active{border-color:rgba(212,168,71,0.4);color:#f0c060;background:rgba(212,168,71,0.08);}
.cc-filter-btn b{color:#eef2ff;font-weight:600;}

.cc-dropdown{position:absolute;top:calc(100% + 4px);left:0;min-width:180px;background:#0d1117;border:1px solid rgba(212,168,71,0.25);border-radius:10px;padding:4px;z-index:999;box-shadow:0 8px 32px rgba(0,0,0,0.6);animation:ccFadeIn .15s ease;}
...
.cc-commit-btn{background:linear-gradient(135deg,#d4a847,#f0c060);border:none;border-radius:7px;padding:5px 14px;font-size:11px;font-weight:700;color:#1a1200;cursor:pointer;text-transform:uppercase;letter-spacing:.05em;transition:.2s;box-shadow:0 2px 8px rgba(212,168,71,0.3);}
.cc-commit-btn:hover{transform:translateY(-1px);box-shadow:0 4px 16px rgba(212,168,71,0.4);}
...
.cc-metric{background:linear-gradient(135deg,rgba(16,22,40,0.9),rgba(10,14,28,0.9));border-radius:10px;padding:9px 6px;border:1px solid rgba(255,255,255,0.07);text-align:center;transition:.2s;position:relative;overflow:hidden;}
.cc-metric::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;border-radius:10px 10px 0 0;}
...
.cc-chart-box{background:linear-gradient(135deg,rgba(16,22,40,0.9),rgba(10,14,28,0.9));border-radius:12px;border:1px solid rgba(255,255,255,0.07);padding:11px;flex:1;display:flex;flex-direction:column;gap:6px;overflow:hidden;}
...
.cc-table-wrap{background:linear-gradient(135deg,rgba(16,22,40,0.9),rgba(10,14,28,0.9));border-radius:12px;border:1px solid rgba(255,255,255,0.07);overflow:hidden;flex-shrink:0;}
.cc-table{width:100%;border-collapse:collapse;font-size:11px;}
.cc-table th{padding:7px 10px;color:#4a5568;font-weight:700;text-align:left;border-bottom:1px solid rgba(255,255,255,0.06);background:rgba(2,4,10,0.6);font-size:9px;text-transform:uppercase;letter-spacing:.6px;}
...
.cc-ps-hero{background:linear-gradient(135deg,rgba(20,28,50,0.95),rgba(16,22,40,0.95));border-radius:14px;border:1px solid rgba(212,168,71,0.25);padding:16px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;position:relative;overflow:hidden;}
.cc-ps-hero::before{content:'';position:absolute;top:-40px;right:-40px;width:120px;height:120px;background:radial-gradient(circle,rgba(212,168,71,0.08),transparent 70%);pointer-events:none;}

.cc-history{background:linear-gradient(135deg,rgba(16,22,40,0.9),rgba(10,14,28,0.9));border-radius:14px;border:1px solid rgba(255,255,255,0.07);overflow:hidden;flex:1;display:flex;flex-direction:column;min-height:0;}
.cc-history-hdr{padding:9px 13px;font-size:9px;color:#4a5568;border-bottom:1px solid rgba(255,255,255,0.06);background:rgba(2,4,10,0.5);font-weight:700;text-transform:uppercase;letter-spacing:.6px;display:flex;justify-content:space-between;align-items:center;flex-shrink:0;}
...
.cc-update-bar{background:linear-gradient(135deg,rgba(16,22,40,0.9),rgba(10,14,28,0.9));border-radius:12px;border:1px solid rgba(255,255,255,0.07);padding:9px 13px;display:flex;align-items:center;justify-content:space-between;flex-shrink:0;}
...
const CommandCenter: React.FC<CommandCenterProps> = ({ currentUser, apiBase }) => {
  ...
  useEffect(() => {
    const handler = () => { setShowUserDrop(false); setShowWeekDrop(false); setShowMonthDrop(false); };
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const msg = e?.name === 'AbortError' ? 'Request timed out (30s)' : e?.message || 'Network error';
  ...
  const filteredTasks = tasks.filter(t => {
    if (selectedUser !== 'all') {
      const taskEmail = t.assignedTo?.toLowerCase();
      const user = users.find(u => u._id === selectedUser || u.email === selectedUser);
      if (!user) return false;
      if (taskEmail !== user.email?.toLowerCase()) return false;
    }
    ...
    if (selectedWeek !== 'all') {
      const d = new Date(t.createdAt || t.dueDate || '');
      if (getWeekNumber(d) !== selectedWeek) return false;
    }
    return true;
  });

  // ── Per-selected-user avg score ─────────────────────────────────────────────
  const selectedUserObj = selectedUser !== 'all' ? users.find(u => u._id === selectedUser || u.email === selectedUser) : null;
  const userScoredTasks = selectedUser !== 'all'
    ? filteredTasks.filter(t => t.scoreData?.percentScore != null)
    : [];
  const userAvgScore = userScoredTasks.length
    ? (userScoredTasks.reduce((s, t) => s + (t.scoreData?.percentScore ?? 0), 0) / userScoredTasks.length).toFixed(1)
    : null;
  ...
  const METRICS = [
    { label: 'Approved', value: approved, color: '#4ade80', grad: '#4ade80' },
    { label: 'Pending', value: pendingCnt, color: '#fbbf24', grad: '#fbbf24' },
    { label: 'Assigned', value: assigned, color: '#60a5fa', grad: '#60a5fa' },
    { label: 'Avg Score', value: avgScore, color: '#c4b5fd', grad: '#c4b5fd' },
    { label: 'Rework', value: String(rework).padStart(2, '0'), color: '#fb7185', grad: '#fb7185' },
    { label: 'In TAT', value: `${inTatPct}%`, color: '#2dd4bf', grad: '#2dd4bf' },
    { label: 'Out TAT', value: `${outTatPct}%`, color: '#f87171', grad: '#f87171' },
  ];
        ...
        <main className="cc-main">
          ...
          <section className="cc-left">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div style={{ background: 'rgba(10,15,30,0.9)', padding: '4px 10px', borderRadius: 7, border: '1px solid rgba(212,168,71,0.2)', display: 'flex', alignItems: 'center', gap: 5 }}>
                <span style={{ fontSize: 11 }}>🕐</span>
                <span style={{ fontSize: 11, fontFamily: 'JetBrains Mono,monospace', fontWeight: 600, color: '#f0c060' }}>{timerDisplay}</span>
              </div>
            </div>

            <div className="cc-video-main">
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, opacity: .6 }}>
              ...
              <div style={{ position: 'absolute', bottom: 8, left: 8, background: 'rgba(0,0,0,0.8)', padding: '3px 9px', borderRadius: 5, fontSize: 10, color: '#eef2ff', border: '1px solid rgba(255,255,255,0.1)' }}>{currentUser?.name || 'Supremo'} · Speaking</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 6 }}>
              {[
                { label: 'Total', value: tasks.length, color: '#afc6ff' },
                { label: 'Breached', value: tasks.filter(t => t.tatBreached).length, color: '#f87171' },
                { label: 'Team', value: users.length, color: '#4ade80' }
              ].map(({ label, value, color }) => (
                <div key={label} style={{ background: 'linear-gradient(135deg,rgba(16,22,40,0.8),rgba(10,14,28,0.8))', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 8, padding: '8px', textAlign: 'center' }}>
            ...
            <div className="cc-toolbar">
              ...
              <button style={{ marginLeft: 4, background: 'linear-gradient(135deg,#dc2626,#ef4444)', border: 'none', borderRadius: 7, padding: '0 14px', height: 30, color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer', boxShadow: '0 2px 8px rgba(220,38,38,0.3)' }}>End</button>
            </div>
          </section>
          ...
          <section className="cc-right">
            ...
            <div className="cc-filters" onClick={e => e.stopPropagation()}>
                ...
                {showWeekDrop && (
                  <div className="cc-dropdown">
                    ...
                    {availableWeeks.map(w => (
                      <div key={w} className={`cc-dropdown-item${selectedWeek === w ? ' selected' : ''}`} onClick={() => { setSelectedWeek(w); setShowWeekDrop(false); }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                {showMonthDrop && (
                  <div className="cc-dropdown">
                    ...
                    <div className="cc-dropdown-divider" />
                    {MONTHS.map((m, i) => (
                      <div key={m} className={`cc-dropdown-item${selectedMonth === i ? ' selected' : ''}`} onClick={() => { setSelectedMonth(i); setShowMonthDrop(false); }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                {showUserDrop && (
                  <div className="cc-dropdown" style={{ minWidth: 220 }}>
                    <div className={`cc-dropdown-item${selectedUser === 'all' ? ' selected' : ''}`} onClick={() => { setSelectedUser('all'); setShowUserDrop(false); }}>
                      <span>All Users</span>
                      <span style={{ fontSize: 9, color: '#4a5568' }}>{tasks.length} tasks</span>
                    </div>
                    <div className="cc-dropdown-divider" />
                    {users.filter(u => u.role !== 'superadmin').map((u, i) => {
                      const uTasks = tasks.filter(t => t.assignedTo?.toLowerCase() === u.email?.toLowerCase());
                      const uScored = uTasks.filter(t => t.scoreData?.percentScore != null);
                      const uAvg = uScored.length ? (uScored.reduce((s, t) => s + (t.scoreData?.percentScore ?? 0), 0) / uScored.length).toFixed(0) : null;
                      return (
                        <div key={u._id} className={`cc-dropdown-item${selectedUser === u._id || selectedUser === u.email ? ' selected' : ''}`} onClick={() => { setSelectedUser(u._id || u.email); setShowUserDrop(false); }}>
                          ...
                          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                            {uAvg && <span style={{ fontSize: 9, color: '#4ade80', fontFamily: 'monospace' }}>{uAvg}%</span>}
                            <span style={{ fontSize: 9, color: '#4a5568' }}>{uTasks.length}t</span>
                          </div>
                        </div>
                      );
                    })}
            ...
            {selectedUserObj && userAvgScore && (
              <div className="cc-user-score-card">
                ...
                <div>
                  <div style={{ fontSize: 11, color: '#eef2ff', fontWeight: 600 }}>{selectedUserObj.name}</div>
                  <div style={{ fontSize: 9, color: '#8b9ab8', marginTop: 1 }}>{userScoredTasks.length} scored tasks · {filteredTasks.length} total</div>
                </div>
            ...
            <div className="cc-tabs">
              ...
              <span style={{ marginLeft: 'auto', fontSize: 9, color: '#4a5568', alignSelf: 'center', fontFamily: 'monospace', paddingRight: 4 }}>
                {filteredTasks.length}/{tasks.length} tasks · {lastRefresh.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
              </span>
            </div>
            ...
            {activeTab === 'overview' && (
              <>
                ...
                <div className="cc-metrics">
                  {METRICS.map(({ label, value, color, grad }) => (
                    <div key={label} className="cc-metric" style={{ '--grad': grad } as any}>
                    ...
                    {BARS.map(({ label, val, pct, color }) => (
                      <div key={label} className="cc-bar-row">
                        ...
                        <div className="cc-bar-track">
                          <div className="cc-bar-fill" style={{ width: `${Math.max(pct, 1)}%`, background: `linear-gradient(90deg,${color}88,${color})` }} />
                        </div>
                ...
                <div className="cc-table-wrap">
                  <div style={{ padding: '9px 12px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(2,4,10,0.5)' }}>
                    <span style={{ fontWeight: 600, fontSize: 12, color: '#eef2ff' }}>
                      {selectedUser !== 'all' ? `${selectedUserObj?.name}'s Tasks` : 'All Tasks'}
                    </span>
                    <span style={{ fontSize: 10, color: '#4a5568' }}>Page {taskPage} of {totalPages} · {filteredTasks.length} total</span>
                  </div>
                  {loadingData ? (
                    <div style={{ padding: 20, textAlign: 'center', fontSize: 11, color: '#fbbf24' }}>⏳ Loading tasks…</div>
                  ) : (
                    <>
                      <table className="cc-table">
                        <thead>
                          <tr>{['Task', 'Assigned To', 'Status', 'Score', 'TAT'].map(h => <th key={h}>{h}</th>)}</tr>
                        </thead>
                        <tbody>
                          {pagedTasks.map((task, idx) => {
                            ...
                            return (
                              <tr key={task._id}>
                                <td style={{ color: '#eef2ff', fontWeight: 500, maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={task.title}>{task.title}</td>
                                <td>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                    <span className="cc-av" style={{ width: 18, height: 18, background: col.bg, color: col.c }}>{ini}</span>
                                ...
                                <td>
                                  <span style={{ color: task.tatBreached ? '#f87171' : '#4ade80', fontWeight: 600, fontSize: 10, background: task.tatBreached ? 'rgba(244,63,94,0.08)' : 'rgba(34,197,94,0.08)', padding: '2px 6px', borderRadius: 4 }}>
                                    {task.tatBreached ? 'Out TAT' : 'In TAT'}
                                  </span>
                                </td>
                              </tr>
                            );
                          ...
                          {pagedTasks.length === 0 && (
                            <tr><td colSpan={5} style={{ textAlign: 'center', color: '#4a5568', padding: 24 }}>No tasks found for selected filters</td></tr>
                          )}
                        </tbody>
                      </table>
                      {totalPages > 1 && (
                        <div className="cc-pagination">
                          ...
                          {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                            const start = Math.max(1, Math.min(taskPage - 2, totalPages - 4));
                            const page = start + i;
                            return page <= totalPages ? (
                              <button key={page} className={`cc-page-btn${taskPage === page ? ' active' : ''}`} onClick={() => setTaskPage(page)}>{page}</button>
                            ) : null;
                          })}
